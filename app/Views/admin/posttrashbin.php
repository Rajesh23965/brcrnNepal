<div class="card bg-light">
    ...
    <div class="card-body">
        <div class="py-2">
        <a href="<?=base_url() . 'view-all-post-list'?>" class="btn btn-outline-primary my-2">Back</a>
            <form action="<?=base_url() . 'delete-multiple-selected-post-permanenlty'?>" method="post">
                ...
                <div class="d-flex flex-column">
                    ...
                    <div class="col p-0 m-0 d-flex gap-3 justify-content-between">
                        ...
                        <div class="">
                            <a href="<?php echo base_url().'delete-all-post-permanently'?>" class="btn btn-outline-danger">Delete All</a>
                        </div>
                    </div>
                    <hr>
        ...
        <table id="myTable" class="table table-striped border-right-1">
            ...
            <tbody>
                <?php foreach ($getDeletePosts as $pl): ?>
                <tr>
                    <td><input class="checkBoxClass selectItem" type="checkbox" class="ml-1 mr-2" value="<?php if (!empty($pl['post_id'])): echo $pl['post_id'];
endif?>"></td>
                    <td><?php if (!empty($pl['post_id'])): echo $pl['post_title'];
endif?></td>
                    <td>ithome</td>
                    <td><?php if (!empty($pl['post_id'])): if ($pl['status_id'] == 1): echo "<p class='text-success'>Active</p>";
    else:echo "<p class='text-danger'>Deactive</p>";
    endif;
endif?></td>
                    <td><?php if (!empty($pl['post_id'])): echo $pl['added_on'];
endif?></td>
                    <td class="d-flex">

                        <a href="<?php echo base_url() . 'delete-post-permanently?page_id='.$pl['post_id'] ?? '' ?>"><i
